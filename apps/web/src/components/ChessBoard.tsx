...
export function getMoveInput(move: Pick<SerializableMove, 'from' | 'to' | 'promotion'>) {
...
export const ChessBoard = memo(
  ({
    ...
    const isFlipped = useAppSelector((state) => state.chessBoard.isBoardFlipped);
    ...
    useEffect(() => {
      if (myColor === 'b') {
        dispatch(setIsBoardFlipped(true));
      }
    }, [dispatch, myColor]);
    ...
    useEffect(() => {
      if (userSelectedMoveIndex !== null) {
        chess.reset();
        moves.forEach((move) => {
          chess.move(getMoveInput(move));
        });
        setBoard(chess.board());
        dispatch(setUserSelectedMoveIndex(null));
      } else {
        setBoard(chess.board());
      }
    }, [chess, dispatch, moves, setBoard, userSelectedMoveIndex]);
                        ...
                        onClick={() => {
                          ...
                          if (!from) {
                            ...
                            try {
                              ...
                              if (moveResult) {
                                const serializedMove = toSerializableMove(moveResult);
                                new Audio('/move.wav').play();

                                if (moveResult?.captured) {
                                  new Audio('/capture.wav').play();
                                }
                                dispatch(appendMove(serializedMove));
                                setFrom(null);
                                setLegalMoves([]);
